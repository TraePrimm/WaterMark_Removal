...
class WatermarkDataset(Dataset):
    VWGAN_MODE = (True, True, False, False, False, False)
    CNN_MODE = (False, False, True, True, True, False)
    YOLO_MODE = (False, True, False, False, False, True)
    def __init__(self, data_dir, mode, *, format_bb=False, scale_im=True):
    ...
    def __len__(self):
        return self.bb_table.shape[0]

    def __getitem__(self, idx):
        im_id = self.bb_table[idx][0]
        ...
        if self.mode[-1]:
            ...
            if self.scale_im:
                ...
                bbox = np.array([y1_resized, y2_resized, x1_resized, x2_resized])
            else:
            ...
            # debugging
            # print(f'idx {idx}: {img_name}, img_size: {original_size}, bounding_box (y1, y2, x1, x2): '
            #   f'{self.bb_table[idx][1:]}, resized_bbox: {bbox}')
    ...
    def process_bbox(self, bbox):
        ...
        ix, iy = min(int(cx), 7), min(int(cy), 7)  # Ensure the index is within the bounds of the 8x8 grid
